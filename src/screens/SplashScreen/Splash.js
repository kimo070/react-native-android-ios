...
@observer export default class Splash extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false
    ...
    componentDidMount() {
        NetInfo.isConnected.addEventListener(
            'connectionChange',
            (isConnected) => {
                isConnected ? this.splash(false) : this.alert()
            }
        );
    }
                        ...
                        <View style={styles.IndicatorCon}>
                            {
                                this.state.loading === true ?
                                    <ActivityIndicator size={INDICATOR_SIZE} color='#e52d27' animating={true}
